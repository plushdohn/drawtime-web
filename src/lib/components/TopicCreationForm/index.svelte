  ...
  >
    ...
    <div class="flex">
      ...
      {#if $form.words.length > 0}
        <button
          on:click={onBatchRemoveWords}
          type="button"
          class="bg-zinc-600 hover:bg-zinc-500 focus:bg-zinc-500 text-white w-24 ml-2 rounded-sm flex justify-center items-center"
        >
          <DeleteSweepIcon class="w-7 fill-white" />
        </button>
      {/if}
    </div>
  </div>
